...
class DataDriftAnalyser(AnalyserBB):
    ...
    def __init__(self, input_bb: BuildingBlock, update_interval=1000, fixed_stops=None):
        ...
        self._training_histograms_by_syscalls: typing.Dict[
            int, Histogram] = {}  # stores copies of the histograms at a certain syscall value
    ...
    def _make_stats(self) -> typing.Union[typing.List[dict], dict]:
        ...
        # see get_stats()
        self._training_histograms_by_syscalls[self._current_i] = self._train_histogram.copy()
        return None

    def get_stats(self):
        print("#ngrams in test set: %s" % self._test_histogram.unique_elements())
        print("#ngrams in complete train set: %s" % self._train_histogram.unique_elements())
        stats = []
    ...
    def _calc_data_drift_measures(self, train_hist, test_hist):
        ...
        return {
            ...
            "ratio_unseen_unique_test_ngrams": unseen_unique_test_ngrams / test_hist.unique_elements()
        }