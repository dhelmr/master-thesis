...
class NgramThreadAnalyser(AnalyserBB):
    ...
    def _add_distribution_stats(self, stats, prefix: str, dists, n_classes):
        ...
        for _, dist in dists.items():
            ...
            self._update_deviation_stats(
                stats, prefix="%s_norm_entropy" % prefix, items=entropies
            )
        ...
        self._update_deviation_stats(
            stats, prefix="%s-simpson-index" % prefix, items=simpson_indexes
        )
        self._update_deviation_stats(
            stats, prefix="%s-gini-coeff" % prefix, items=gini_coeffs
        )
        self._update_deviation_stats(
            stats, prefix="%s-zipf_a" % prefix, items=zipf_params_a
        )
        self._update_deviation_stats(
            stats, prefix="%s-zipf_loc" % prefix, items=zipf_params_loc
        )
        self._update_deviation_stats(
            stats, prefix="%s-simpson-index" % prefix, items=simpson_indexes
        )
    ...
    def _add_pdist_stats(self, stats, prefix, matrix):
        for metric in [
            ...
            self._add_matrix_stats(
                stats, prefix=f"{prefix}-{metric}", matrix=distance_matrix
            )
    ...
    def _add_pca_stats(self, stats, prefix, matrix):
        ...
        pca_stats = {
            "pca-noise-var": pca.noise_variance_,
        }
