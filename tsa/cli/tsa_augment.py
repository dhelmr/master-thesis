    ...
    def augment(self, data: PerformanceData) -> pandas.DataFrame:
        ...
        for sc in tqdm(data.get_scenarios()):
            ...
            df.drop_duplicates(
                ignore_index=True,
                inplace=True,
                subset=[data.syscalls_col, data.scenario_col],
            )
...
class FeatureTransform:
    ...
    def augment(self, data: PerformanceData) -> pandas.DataFrame:
        for f1 in tqdm(data.feature_cols()):
            data.df[f"log({f1})"] = data.df[f1].apply(
                lambda x: math.log(abs(x)) if x != 0 else -100
            )
            ...
            # data.df[f"e({f1})"] = data.df[f1].apply(lambda x: math.exp(x))
        ...
        for f1 in tqdm(data.feature_cols()):
            for f2 in data.feature_cols():
                if (f2, f1) not in processed_comb_features:
                    data.df[f"({f1})x({f2})"] = data.df[f1] * data.df[f2]
...
AUGMENTORS = {
    cls.__name__: cls for cls in [TimeSeries, FeatureTransform, FeatureCombine]
}
...
class TSAAugmentSubCommand(SubCommand):
    ...
    def make_subparser(self, parser: ArgumentParser):
        ...
        parser.add_argument(
            "--augmentor", "-a", required=True, choices=AUGMENTORS.keys()
        )
