...
class RunSubCommand(SubCommand):
    ...
    def exec(self, args, parser):
        ...
        if args.continue_experiment is not None:
            start_at = get_next_iteration(mlflow_client, experiment, args.continue_experiment, args.experiment)
...
def get_next_iteration(mlflow_client, experiment, mode, experiment_name):
    ...
    if mode == "finished":
        run_id = last_successful_run_id(mlflow_client, experiment_name)
    elif mode == "running":
        run_id = last_running_run_id(mlflow_client, experiment_name)
        if run_id is None:
            print("No running run found; find last successfully finished run.")
            run_id = last_successful_run_id(mlflow_client, experiment_name)
    elif mode == "next-free":
        return ExperimentChecker(experiment).next_free_iteration(experiment_name)
    else:
...
def make_experiment(path, mlflow_client):
    with open(path) as f:
