...
def last_successful_run_id(mlflow_client: MlflowClient, experiment_name: str) -> str:
    ...
    for r in mlflow_client.search_runs(
...
class RunSubCommand(SubCommand):
    ...
    def make_subparser(self, parser):
        ...
        parser.add_argument(
            "--number-runs",
            required=False,
            help="Only iterate for specific number of runs",
            type=int,
            default=None,
        )
    ...
    def exec(self, args, parser, unknown_args):
        ...
        if args.experiment is None and args.continue_experiment is True:
            print(
                "If --continue-experiment is set, an experiment must be specified with -e."
            )
            parser.print_help()
            exit(1)
        ...
        experiment = make_experiment_from_path(
            args.config, mlflow_client, args.experiment
        )

        experiment_start_args = {"num_runs": args.number_runs, "dry_run": args.dry_run}
        if args.continue_experiment is not None:
            start_at = get_next_iteration(
                mlflow_client, experiment, args.continue_experiment
            )
            if start_at is None:
                return
...
def make_experiment(parameter_config, mlflow_client, name):
    ...
    if exp_mode == "normal":
        experiment = Experiment(
            parameter_config, mlflow=mlflow_client, mlflow_name=name
        )
