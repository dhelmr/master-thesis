...
class TSACrossValidateSubCommand(SubCommand):
    def __init__(self):
        super().__init__(
            "tsa-cv", "cross validate performance predictor", expect_unknown_args=True
        )
        import warnings
    ...
    def exec(self, args, parser, unknown_args):
        ...
        for predictor_name in args.predictor:
            ...
            cv = CV(data, predictor=predictor, cv_leave_out=args.leave_out)
...
def load_data(
    path: str, scenario_col, feature_cols: Optional[List[str]], skip_features=[]
) -> PerformanceData:
    ...
    if feature_cols is None:
        ...
        feature_cols = [
            c
            for c in df.columns
            if str(c) not in NON_FEATURE_COLS
            and str(c) not in skip_features
            and "." not in str(c)
        ]
    ...
    return PerformanceData(df=df, feature_cols=feature_cols, scenario_col=scenario_col)
