...
class TSAConcatSubCommand(SubCommand):
    ...
    def exec(self, args, parser, unknown_args):
        ...
        for i_file in args.input:
            ...
            df.drop(columns=["parameter_cfg_id", "run_id"], inplace=True)
            if merged_df is None:
                merged_df = df
                continue
            merged_df = pandas.merge(merged_df, df, on=["scenario", "permutation_i", "iteration", "is_last", "syscalls", "num_attacks"]+args.common)
