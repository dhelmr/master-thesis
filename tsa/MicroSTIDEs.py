...
class MicroSTIDEs(BuildingBlock):
    ...
    def __init__(self, input: BuildingBlock, n_models=20, stream_sum_window_len = 1000, aggregation="mean"):
        super().__init__()
        # parameter
        self._input = input
    ...
    def _get_model(self, syscall):
        trace_id = process_thread_id(syscall)
        if trace_id not in self._trace_ids:
            self._trace_ids[trace_id] = random.randrange(self._n_models)
        model_id = self._trace_ids[trace_id]
        return self._normal_models[model_id]
