...
class ContaminatedDataLoader2019(TsaBaseDataloader):
    ...
    def training_data(self) -> list:
        ...
        recordings = list(
            yield_successively(
                [self._normal_recordings[:TRAIN_OFFSET], self._contaminated_recordings],
    ...
    def test_data(self) -> list:
        ...
        recordings = list(
            yield_successively(
                [normal_test_data, self._exploit_recordings], limit=self._test_size
            )
        )
    ...
    def _extract_recordings(self):
        ...
        with open(self._runs_path, "r") as runs_csv:
            ...
            for recording_line in recording_reader:
                recording = Recording2019(
                    recording_line, self.scenario_path, self._direction
                )
        ...
        training_exploit_lines, test_exploit_lines = split_list(
            exploit_recording_lines, self._cont_ratio
        )
        ...
        training_exploit_lines = random_permutation(
            training_exploit_lines, self._num_attacks, self._permutation_i
        )
        ...
        for recording_line, _ in training_exploit_lines:
            ...
            self._contaminated_recordings.append(
                Recording2019(recording_line, self.scenario_path, self._direction)
            )
    ...
    def distinct_syscalls_training_data(self) -> int:
        ...
        if self._distinct_syscalls is not None:
            ...
            with open(self.scenario_path + json_path, "w") as distinct_syscalls:
                json.dump(
                    {"distinct_syscalls": self._distinct_syscalls}, distinct_syscalls
                )
            return self._distinct_syscalls
