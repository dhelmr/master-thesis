...
class NgramAnalyserTest(unittest.TestCase):
    ...
    def test_ngram_analyser(self):
        ...
        pprint.pprint(by_ngram_size)
        self.assertSetEqual(set(by_ngram_size.keys()), {1, 2, 3, 4, 5})
        self.assertEqual(by_ngram_size[1]["density"], 1)
        self.assertEqual(by_ngram_size[1]["unique"], 4)  # a,b,c
        self.assertEqual(by_ngram_size[2]["unique"], 9)  # ab, bc, ca, aa, bb, ba, ac, ad, dd
        self.assertEqual(by_ngram_size[3]["unique"], 11)  # abc, bca, cab, caa, aab, abb, bba, bac, aca, cad, add
        self.assertEqual(by_ngram_size[5]["unique"], 11)
        ...
        self.assertAlmostEqual(by_ngram_size[2]["variability"], 0.941734,
                               places=5)  # entropy/max entropy for n=2: 2.0692/log(9)
        self.assertAlmostEqual(by_ngram_size[2]["u/t"], 0.6428, places=2)
