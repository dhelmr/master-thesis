...
function make-table { # $1 = basename file prefix (e.g. perf-ngrams-augmented) $2=best baseline f1 score
  ...
  pd -i $TMP_DIR/results.csv --sort "mean.precision" --descending --head 3 --only max_depth features "mean precision" "mean tnr" "mean f1-score" "precision gain" --rename "mean.precision::mean precision" "mean.tnr::mean tnr" "mean.f1_score::mean f1-score" "gain.precision::precision gain" -q "max_depth == 2" -o $RSQ_DIR/$1-best-depth=2.csv
  pd -i $TMP_DIR/results.csv --sort "mean.precision" --descending --head 3 --only max_depth features "mean precision" "mean tnr" "mean f1-score" "precision gain"  --rename "mean.precision::mean precision" "mean.tnr::mean tnr" "mean.f1_score::mean f1-score" "gain.precision::precision gain" -q "max_depth == 3" -o $RSQ_DIR/$1-best-depth=3.csv
  pd -i $TMP_DIR/results.csv --sort "mean.precision" --descending --head 3 --only max_depth features "mean precision" "mean tnr" "mean f1-score" "precision gain"  --rename "mean.precision::mean precision" "mean.tnr::mean tnr" "mean.f1_score::mean f1-score" "gain.precision::precision gain" -q "max_depth == 5" -o $RSQ_DIR/$1-best-depth=5.csv
  ...
  sed 's/;/; /g' -i "$LATEX_FILE"
  sed 's/tabular}{rlrr}/tabularx}{\\linewidth}{c|X|c|c}/g' -i "$LATEX_FILE"
  sed 's/tabular}/tabularx}/g' -i "$LATEX_FILE"
  sed 's/)/)\\allowbreak /g' -i "$LATEX_FILE"
  sed 's/_/_\\allowbreak /g' -i "$LATEX_FILE"
}
